...
func TestFillOneAllHappy(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		},
		{
			...
			expectAutofillError: api.ErrInsufficientForNeeds,
		...
		},
		{
			...
			pool:                test.Usermap(),
			chosen:              test.Usermap(test.UserMobile2()),
			expectAutofillError: api.ErrInsufficientForSize,
			expectUnmetCapacity: 1,
		},
		{
...
func makeTestAutofill(t testing.TB, size int, needs store.Needs,
	pool api.UserMap, chosen api.UserMap, shiftNumber int) (*autofill, error) {
